...
func (a *application) getNewestSensorDailyReading(w http.ResponseWriter, r *http.Request) {
	user := a.contextGetUser(r)
	if user.IsAnonymous() {
		a.invalidAuthenticationTokenResponse(w, r)
		return
	...
	sensorValues, err := a.models.SensorValue.GetNowForDevice(input.SensorDeviceID, "day")
	if err != nil {
		a.serverErrorResponse(w, r, err)
		return
	}
